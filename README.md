...
dssh 是一个轻量、跨平台（Windows / macOS）的 SSH 管理桌面应用，使用 Rust + Tauri 2 + React + TypeScript 构建。
...
**终端与工作区**
- **分屏**：同一标签内最多 4 个终端，横向或纵向排列，可拖拽调整比例；新分出的面板**自动连接到当前所在主机**（本地面板则开本地 shell）
- **标签页**：终端与 SFTP 统一在顶部标签条，支持拖拽重排、中键关闭
...
**文件、对象存储与运维**
- SFTP 文件浏览与传输（浏览、上传 / 下载、拖拽上传、目录打包下载、路径自动补全与缓存），以标签页形式按主机打开
...
| --- | --- |
| 桌面框架 | Tauri 2 |
| 后端 | Rust edition 2024（`russh` SSH + 主机密钥 TOFU、`russh-sftp` 文件传输、`portable-pty` 本地终端、`rusqlite` 存储、`aws-sdk-s3`、`argon2` + `aes-gcm` 加密导出） |
