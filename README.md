...
界面采用**工作区布局**——左侧活动栏、会话树侧栏、顶部标签页、右侧可停靠面板（AI / 主机工具）——配以冷黑、雾白与克制冰蓝的 **Graphite Glass** 主题，chrome 与浮层带轻量玻璃层次，风格紧凑、接近原生 IDE。
...
- **终端外观**：可选背景图片、磨砂开关与模糊度、背景不透明度调节；仅在当前系统 WebView 支持背景模糊时显示图片
