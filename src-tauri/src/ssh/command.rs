...
/// Run an SSH exec command with a hard combined stdout/stderr byte cap, so a
/// faulty remote process cannot grow the desktop process without bound.
async fn run_ssh_command_with_limit(
    profile: SshProfile,
    command: String,
    timeout_secs: u64,
