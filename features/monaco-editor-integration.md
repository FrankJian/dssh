...
> 状态：提案，未排期。本文定义以 Monaco 升级远程文本编辑体验的边界与实施约束；目录树、远程文件操作与图片 / Markdown 预览不属于 Monaco 的职责。
...
- 保持现有 SFTP 读写、主机密钥 TOFU、文件标签、未保存状态、保存确认和 5 MiB 文本限制。
...
- 默认仅启用常见语言和必要 worker；最小化首包、内存与 worker 数量。大型文件仍受后端 5 MiB 限制。
