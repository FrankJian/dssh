  ...
  return <article className="conn-card" data-variant={variant} onDoubleClick={canOpen ? onOpen : undefined}>
    ...
    <div className="conn-card__actions">
      <button
        ...
        disabled={!canOpen}
        onClick={onOpen}
        title={profile.selectedContexts.length > 0 ? "打开 Kubernetes 工作区" : "请先选择 Kubernetes context"}
        type="button"
      >
        <Icon name="play" height="14" width="14" />
        <span>{profile.selectedContexts.length > 0 ? "打开" : "选择 context"}</span>
      </button>
